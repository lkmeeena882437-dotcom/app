    ...
    <section className="relative py-24 sm:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
          ...
          <p className="ink-mute max-w-md text-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
          {REELS.map((r) => (
            <div
              key={r.id}
              className="group relative aspect-[9/16] rounded-xl overflow-hidden bg-[#F5F5F7] border border-[#E5E5EA] cursor-pointer"
            >
              ...
              <div className="absolute inset-0 grid place-items-center opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="w-12 h-12 rounded-full bg-white/85 backdrop-blur grid place-items-center border border-white">
                  <Play className="w-4 h-4 fill-[#1D1D1F] text-[#1D1D1F]" />
                </div>
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-3 text-white">
                <div className="flex items-center justify-between text-xs">
                  <span>{r.user}</span>
                  <span className="flex items-center gap-1 opacity-80"><Heart className="w-3 h-3"/> {Math.floor(Math.random()*9 + 1)}.{Math.floor(Math.random()*9)}k</span>
                </div>
                <p className="mt-1 text-[11px] opacity-90 line-clamp-1">{r.caption}</p>
              </div>
            </div>
          ))}
        </div>
